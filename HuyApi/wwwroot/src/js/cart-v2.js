...
function updateSummary(total) {
    ...
    if (progress && status) {
        ...
        if (total >= 200000) {
            status.innerHTML = '<b class="text-success">🎉 Miễn phí vận chuyển cho đơn hàng này!</b>';
        } else {
...
function proceedToCheckout() {
    ...
    if (cart.length === 0) {
        alert("Giỏ hàng của ông đang trống mà, mua thêm sách đi!");
        return;
    }
    
    // Chuyển hướng sang trang thanh toán chuyên nghiệp đã làm từ trước
